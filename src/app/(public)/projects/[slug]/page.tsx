...
const TYPE_LABEL: Record<string, string> = { project: "Project", initiative: "Initiative", campaign: "Campaign" };
...
export default async function ProjectDetailPage({ params }: Props) {
  ...
  let related: { id: string; title: string; slug: string; summary: string; programme: { title: string; slug: string } | null }[] = [];
  ...
  try {
    ...
    related = await prisma.project.findMany({
      where: { programmeId: project.programmeId ?? undefined, slug: { not: slug }, status: "published" },
      take: 3,
  ...
  const hasDetails = Boolean(project.location || project.startDate || project.endDate);
    ...
    <>
      ...
      <div className="relative h-64 overflow-hidden bg-forest sm:h-80 lg:h-96">
        ...
        <Container className="relative flex h-full flex-col justify-end pb-8">
          {TYPE_LABEL[project.type] && <Badge variant="default">{TYPE_LABEL[project.type]}</Badge>}
        ...
        <Container>
          <div className="grid gap-10 lg:grid-cols-[1fr_320px]">
            ...
            <div>
              ...
              {updates.length > 0 && (
                <div className="mt-10">
                  <h2 className="mb-6 font-display text-xl font-bold text-forest">Updates</h2>
            ...
            {/* Details sidebar (only when there is real data) */}
            {hasDetails && (
              <aside>
                <div className="rounded-2xl border border-black/5 bg-offwhite p-5">
                  <h3 className="font-display text-sm font-bold text-forest">Details</h3>
      ...
      {related.length > 0 && (
        <section className="bg-mint py-12">
          <Container>
            <h2 className="mb-6 font-display text-xl font-bold text-forest">Related initiatives</h2>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              {related.map((r) => (
                <Link key={r.id} href={`/projects/${r.slug}`} className="group rounded-2xl border border-black/5 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
                  ...
                  <p className="mt-2 text-sm text-muted">{r.summary.length > 100 ? `${r.summary.slice(0, 100)}…` : r.summary}</p>
                </Link>
              ))}
            </div>
          </Container>
